   ...
   */
    ...
    for(int k = 0;k<(n+TILE-1)/TILE;k++){
        if(k * TILE + threadIdx.y < n){
            ...
            int o = index_to_position(index, a_strides, 3);
            a_shared[threadIdx.x][threadIdx.y] = a_storage[o];   
        }
        else
        ...
        if(k * TILE + threadIdx.x < n){
            ...
            index[2] = col;
            int o = index_to_position(index, b_strides, 3);
            b_shared[threadIdx.x][threadIdx.y] = b_storage[o];
        }
        else
        ...
        for(int itr = 0; itr<TILE; itr++){
            res+= (a_shared[threadIdx.x][itr] * b_shared[itr][threadIdx.y]);
        }
        __syncthreads();
    }
    if(row < m && col < p){
        ...
        index[1] = row;
        index[2] = col;
        int o = index_to_position(index, out_strides, 3);
        out[o] = res;
    }
    /// END ASSIGN1_2
}
   ...
   */
    ...
    if(i < out_size){
        ...
        for(int s=0;s<reduce_size;s++){
            ...
            identity = fn(fn_id, identity, a_storage[j]);
        }
        out[i] = identity;
    }
    /// END ASSIGN1_2
