...
class Network(minitorch.Module):
    ...
    def __init__(
        ...
        self.linear_layer1 = Linear(embedding_dim, hidden_dim)
        self.linear_layer2 = Linear(hidden_dim, 1)
        #raise NotImplementedError
        # END ASSIGN1_3
    ...
    def forward(self, embeddings):
        ...
        x = embeddings.mean(1).view(batch, embedding_dim)
    ...
    def train(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for batch_num, example_num in enumerate(
                ...
                loss = -(y * (out.log()) + (1 + y.__neg__()) * ((1 + out.__neg__()).log())).mean()
                #print(loss)
                #
...
def encode_sentiment_data(dataset, pretrained_embeddings, N_train, N_val=0):
    ...
    #unk_embedding = [
    #    0.1 for i in range(pretrained_embeddings.d_emb)
    #]
